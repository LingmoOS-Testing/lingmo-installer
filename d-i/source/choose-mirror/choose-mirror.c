...
static int get_release(struct release_t *release, const char *name) {
	...
	if (base_on_cd && ! manual_entry) {
		 ...
		 * Lingmo-specific change since (a) Debian netinst CDs etc.
...
static int choose_mirror(void) {
	...
	if (! manual_entry) {
		...
		if ((debconf_get(debconf, mir) == 0 &&
		     strcmp(debconf->value, "CC.archive.lingmo.com") == 0) ||
		    debconf_fget(debconf, mir, "seen") != 0 ||
		    strcmp(debconf->value, "true") != 0) {
			countryarchive = get_localized_archive(".archive.lingmo.com");
			if (mirror_root(countryarchive))
				debconf_set(debconf, mir, countryarchive);
			free(countryarchive);
		}
		if (debconf_get(debconf, mir) == 0 &&
		    strcmp(debconf->value, "CC.ports.lingmo.com") == 0) {
			countryarchive = get_localized_archive(".ports.lingmo.com");
			if (mirror_root(countryarchive))
				debconf_set(debconf, mir, countryarchive);
			free(countryarchive);
		}
