...
load_indicator_files (const gchar *directory, GtkWidget *menu)
{
	...
	while ((name = g_dir_read_name (dir))) {
		...
		IndicatorNg *io = indicator_ng_new_for_profile (path, "lingmo-installer", &error);
		if (io) {
...
on_draw(GtkWidget *widget, cairo_t *cr, gpointer userdata) {
	...
	if (!pixbuf) {
		pixbuf = gdk_pixbuf_new_from_file("/usr/share/lxpanel/images/llingmo-background.png", NULL);
	}
	if (!pixbuf) {
		pixbuf = gdk_pixbuf_new_from_file("/usr/share/themes/Greybird/lingmo-installer-panel-bg.png", NULL);
	}
	if (!pixbuf) {
		pixbuf = gdk_pixbuf_new_from_file("/usr/share/budgie-desktop/lingmo-installer-panel-bg.png", NULL);
	}
	if (!pixbuf) {
		pixbuf = gdk_pixbuf_new_from_file("/usr/share/lingmo-installer/pixmaps/panel.png", NULL);
	}
	if (pixbuf) {
