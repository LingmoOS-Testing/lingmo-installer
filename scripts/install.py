...
class Install(install_misc.InstallBase):
    ...
    def __init__(self):
        ...
        with open('/var/lib/lingmo-installer/started-installing', 'a'):
            pass
        ...
        if os.path.isdir('/rofs'):
            ...
            self.source = '/var/lib/lingmo-installer/source'
        ...
        self.keep_ad_tools()
        if self.db.get('lingmo-installer/install/generate-blacklist') == 'true':
            self.db.progress('START', 0, 100, 'lingmo-installer/install/title')
            self.db.progress('INFO', 'lingmo-installer/install/blacklist')
    ...
    def run(self):
        ...
        self.db.progress('INFO', 'lingmo-installer/install/mounting_source')

        if self.source == '/var/lib/lingmo-installer/source':
            self.mount_source()
        ...
        if self.target != '/':
            ...
            if self.db.get('lingmo-installer/download_updates') == 'true':
                cmd = ['/usr/share/lingmo-installer/update-apt-cache']
            ...
            try:
                ...
                if e.errno in (errno.ENOENT, errno.EIO, errno.EFAULT,
                    ...
                    error_template = ('lingmo-installer/install/copying_error/%s' %
                                      error_template)
                    ...
                    error_template = 'lingmo-installer/install/copying_error/no_space'
                    self.db.subst(error_template, 'ERROR', str(e))
                    self.db.input('critical', error_template)
                    self.db.go()
                    sys.exit(3)
        ...
        if self.update_proc:
            ...
            syslog.syslog('Terminated lingmo-installer update process.')
    ...
    def find_cd_kernel(self):
        ...
        for prefix in ('vmlinux', 'vmlinuz'):
            for suffix in ('', '.efi', '.efi.signed'):
                kernel = os.path.join(self.live_path, prefix)
                if os.path.exists(kernel + suffix):
                    return kernel

                if subarch:
                    kernel = os.path.join(self.live_path, subarch, prefix)
                    if os.path.exists(kernel + suffix):
                        return kernel

                    kernel = os.path.join(self.live_path,
                                          '%s-%s' % (prefix, subarch))
                    if os.path.exists(kernel + suffix):
                        return kernel
    ...
    def generate_blacklist(self):
        ...
        # Even adding lingmo-installer as a depends to oem-config-{gtk,kde} doesn't
        # appear to force lingmo-installer and libdebian-installer4 to copy all of
    ...
    def copy_all(self):
        ...
        self.db.progress('START', 0, 100, 'lingmo-installer/install/title')
        self.db.progress('INFO', 'lingmo-installer/install/copying')

        fs_size = os.path.join(self.live_path, 'filesystem.size')
        if os.path.exists(fs_size):
            with open(fs_size) as total_size_fp:
                total_size = int(total_size_fp.readline())
        else:
            # Fallback in case an Lingmo derivative forgets to put
            # /live/filesystem.size on the CD, or to account for things
            ...
            # not suck the list into lingmo-installer's memory, and I'm guessing
        ...
        debug = 'LINGMO-INSTALLER_DEBUG' in os.environ
        if self.db.get('lingmo-installer/install/md5_check') == 'false':
            md5_check = False
        else:
            md5_check = True
                    ...
                    if long_enough and time_now - time_last_update >= 2:
                        ...
                        if speed != 0:
                            ...
                            if time_remaining < 60:
                                self.db.progress(
                                    'INFO', 'lingmo-installer/install/copying_minute')
    ...
    def mount_one_image(self, fsfile, mountpoint=None):
        ...
        if mountpoint is None:
            mountpoint = '/var/lib/lingmo-installer/%s' % sysloop
    ...
    def mount_source(self):
        ...
        fs_preseed = self.db.get('lingmo-installer/install/filesystem-images')
        ...
        if fs_preseed == '':
            ...
            fsfiles = [os.path.join(self.live_path, 'filesystem.cloop'),
                       os.path.join(self.live_path, 'filesystem.squashfs'),
    ...
    def select_ecryptfs(self):
        ...
        user-setup-ask runs in lingmo-installer, /target is not yet mounted, but we
...
if __name__ == '__main__':
    ...
    if not os.path.exists('/var/lib/lingmo-installer'):
        os.makedirs('/var/lib/lingmo-installer')
    osextras.unlink_force('/var/lib/lingmo-installer/install.trace')
