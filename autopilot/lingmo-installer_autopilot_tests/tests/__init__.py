...
class Lingmo-installerAutopilotTestCase(Lingmo-installerTestCase):
    ...
    def launch_application(self):
        '''
        Hmm... launch lingmo-installer


        :returns: The application proxy object.
        '''
        my_process = int(os.environ['LINGMO-INSTALLER_PID'])
    ...
    def edlingmo_addon_window_tests(self, unity=False,
                                    gnome=False, ltsp=False):
        """Run Page tests for edlingmo addon page"""
        self._update_current_step('edlingmo-addon_window')
        self._check_navigation_buttons()
        self._update_page_titles()
        add_on_page = self.main_window.select_single(
            'GtkVBox', BuilderName='edlingmo-addon_window')
    ...
    def edlingmo_packages_window_tests(self, ):
        """Run Page tests for edlingmo packages page"""
        self._update_current_step('edlingmo-packages_window')
        self._check_navigation_buttons()
        self._update_page_titles()
