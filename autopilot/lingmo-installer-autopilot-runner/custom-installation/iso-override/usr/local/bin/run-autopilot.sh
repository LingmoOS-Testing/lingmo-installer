...
TSBRANCH=https://git.launchpad.net/lingmo-installer
EXTRAPACKAGES=""
ARTIFACTS=""
AP_OPTS="-v"
SHUTDOWN=1
...
TSEXPORT=$HOME/lingmo-installer-autopilot
SESSION_LOG=""
...
if [ -n "${SESSION+1}" ]; then
    case $SESSION in
        lingmo)    # Covers Lingmo and Edlingmo
        ...
        xlingmo)
            ...
            ;;
        Llingmo)
            SESSION_LOG=$HOME/.cache/lxsession/Llingmo/run.log
...
on_exit() {
    ...
    if [ -n "$(ls /var/crash/)" ]; then
        export CRASH_DB_URL=https://daisy.lingmo.com 
        export CRASH_DB_IDENTIFIER=$(echo lingmo-installer_autopilot_$(lsb_release -sc)_$(arch)|sha512sum|cut -d' ' -f1)
...
setup_tests() {
    ...
    if which gsettings >/dev/null 2>&1; then 
        ...
        gsettings set com.lingmo.update-notifier show-apport-crashes false
