...
#
# Functions useful for the final install.py script and for lingmo-installer
# plugins to use
#
...
from lingmo-installer import misc, osextras
from lingmo-installer.live import get_live

minimal_install_rlist_path = os.path.join(
    '/cdrom',
    get_live('LIVE_MEDIA_PATH', 'live').lstrip('/'),
...
def reconfigure(target, package):
    ...
    subprocess.call(['log-output', '-t', 'lingmo-installer', 'chroot', target,
...
def set_debconf(target, question, value, db=None):
    try:
        if 'LINGMO-INSTALLER_OEM_USER_CONFIG' in os.environ and db:
            dccomm = None
            dc = db
        else:
            dccomm = subprocess.Popen(['log-output', '-t', 'lingmo-installer',
                                       '--pass-stdout',
                                       'chroot', target,
                                       'debconf-communicate',
                                       '-fnoninteractive', 'lingmo-installer'],
...
def record_installed(pkgs):
    ...
    record_file = "/var/lib/lingmo-installer/apt-installed"
...
def query_recorded_installed():
    apt_installed = set()
    if os.path.exists("/var/lib/lingmo-installer/apt-installed"):
        with open("/var/lib/lingmo-installer/apt-installed") as record_file:
...
def record_removed(pkgs, recursive=False):
    ...
    record_file = "/var/lib/lingmo-installer/apt-removed"
    if not os.path.exists(os.path.dirname(record_file)):
        os.makedirs(os.path.dirname(record_file))
    with open(record_file, "a") as record:
        for pkg in pkgs:
...
def query_recorded_removed():
    ...
    if os.path.exists("/var/lib/lingmo-installer/apt-removed"):
        with open("/var/lib/lingmo-installer/apt-removed") as record_file:
            for line in record_file:
...
class DebconfAcquireProgress(AcquireProgress):
    ...
    def start(self):
        if os.environ['LINGMO-INSTALLER_FRONTEND'] != 'debconf_ui':
...
def excepthook(exctype, excvalue, exctb):
    ...
    with open('/var/lib/lingmo-installer/install.trace', 'w') as tbfile:
...
def copy_file(db, sourcepath, targetpath, md5_check):
    while 1:
        ...
        if targethash.digest() != sourcehash.digest():
            error_template = 'lingmo-installer/install/copying_error/md5'
...
class InstallBase:
    def __init__(self):
        ...
        self.live_path = os.path.join(
            '/cdrom', get_live('LIVE_MEDIA_PATH', 'live').lstrip('/'))
    ...
    def warn_broken_packages(self, pkgs, err):
        ...
        self.db.subst('lingmo-installer/install/broken_install', 'ERROR', err)
        self.db.subst('lingmo-installer/install/broken_install', 'PACKAGES', pkgs)
        self.db.input('critical', 'lingmo-installer/install/broken_install')
        self.db.go()
    ...
    def nested_progress_end(self):
        if os.environ['LINGMO-INSTALLER_FRONTEND'] == 'debconf_ui':
            self.db.progress('START', self.start, self.end,
                             'lingmo-installer/install/title')
    ...
    def do_install(self, to_install, langpacks=False):
        ...
        if langpacks:
            self.db.progress('START', 0, 10, 'lingmo-installer/langpacks/title')
        else:
            self.db.progress('START', 0, 10, 'lingmo-installer/install/title')
        self.db.progress('INFO', 'lingmo-installer/install/find_installables')
        ...
        with Cache() as cache:
            ...
            if langpacks:
                fetchprogress = DebconfAcquireProgress(
                    self.db, 'lingmo-installer/langpacks/title', None,
                    'lingmo-installer/langpacks/packages')
                installprogress = DebconfInstallProgress(
                    self.db, 'lingmo-installer/langpacks/title',
                    'lingmo-installer/install/apt_info')
            else:
                fetchprogress = DebconfAcquireProgress(
                ...
                installprogress = DebconfInstallProgress(
                    self.db, 'lingmo-installer/install/title',
                    'lingmo-installer/install/apt_info',
                    'lingmo-installer/install/apt_error_install')
    ...
    def select_language_packs(self, save=False):
        try:
            keep_packages = self.db.get('lingmo-installer/keep-installed')
        ...
        no_install = '/var/lib/lingmo-installer/no-install-langpacks'
        ...
        langpacks_file = '/var/lib/lingmo-installer/langpacks'
