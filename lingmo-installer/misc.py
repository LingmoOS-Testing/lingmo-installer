...
def grub_options():
    """ Generates a list of suitable targets for grub-installer
        @return empty list or a list of ['/dev/sda1','Lingmo Hardy 8.04'] """
    from lingmo-installer.parted_server import PartedServer
...
def remove_os_prober_cache():
    osextras.unlink_force('/var/lib/lingmo-installer/os-prober-cache')
    shutil.rmtree('/var/lib/lingmo-installer/linux-boot-prober-cache',
                  ignore_errors=True)
...
def get_release():
    if get_release.release_info is None:
        ...
        if not get_release.release_info:
            get_release.release_info = ReleaseInfo(name='Lingmo', version='')
    ...
    if not get_release_name.release_name:
        try:
            ...
            syslog.syslog(
                ...
                "/cdrom/.disk/info")
        if not get_release_name.release_name:
            get_release_name.release_name = 'Lingmo'
...
def set_indicator_keymaps(lang):
    ...
    from lingmo-installer import gsettings
    ...
    # Map inspired from that of gfxboot-theme-lingmo that's itself
    ...
    if nodes:
        ...
        # during lingmo-installer-dm, hence if we change that key, we should purge the
        # state that gsd uses, to determine if it should run the
        # conversion. Which are a stamp file, and having the new key set.
        # Ideally, I think lingmo-installer should be more universal and set the new key
...
def install_size():
    ...
    try:
        with open('/cdrom/live/filesystem.size') as fp:
...
def is_removable_device(path):
    ...
    # In lingmo-installer environment / is the installation media
    if mp == "/":
        return True
