...
from lingmo-installer.filteredcommand import FilteredCommand
...
class Install(FilteredCommand):
    def prepare(self, unfiltered=False):
        ...
        install_bootloader_seen = self.db.fget('lingmo-installer/install_bootloader',
                                               'seen')
        if not (automatic_mode and install_bootloader_seen):
            if self.frontend.oem_user_config:
                self.preseed_bool('lingmo-installer/install_bootloader', False)
            else:
                self.preseed_bool('lingmo-installer/install_bootloader', True)

        # for clock-setup
        self.preseed('netcfg/dhcp_ntp_servers', '', seen=False)

        questions = ['^.*/apt-install-failed$',
                     'lingmo-installer/install/copying_error/md5',
                     'lingmo-installer/install/new-bootdev',
                     'CAPB',
                     'ERROR',
                     'PROGRESS']
        return (['/usr/share/lingmo-installer/plugininstall.py'], questions)
    ...
    def error(self, priority, question):
        if question == 'hw-detect/modprobe_error':
            ...
            fatal = False
        elif (question == 'lingmo-installer/install/broken_active_directory' or
              question == 'lingmo-installer/install/broken_luks_add_key'):
            fatal = False
        else:
            fatal = True
    ...
    def run(self, priority, question):
        ...
        elif question == 'lingmo-installer/install/new-bootdev':
