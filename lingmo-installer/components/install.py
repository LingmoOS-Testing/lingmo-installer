...
class Install(FilteredCommand):
    def prepare(self, unfiltered=False):
        reboot = self.db.get('lingmo-installer/reboot')
        if reboot == 'true':
            self.frontend.set_reboot(True)
        else:
            self.frontend.set_reboot(False)

        shutdown = self.db.get('lingmo-installer/poweroff')
        if shutdown == 'true':
        ...
        questions = ['^.*/apt-install-failed$',
                     ...
                     'ERROR',
                     'PROGRESS']
        return (['/usr/share/lingmo-installer/install.py'], questions)
    ...
    def run(self, priority, question):
        if question == 'lingmo-installer/install/copying_error/md5':
