...
from lingmo-installer.misc import drop_privileges, execute_root
...
class BaseFrontend:
    """Abstract lingmo-installer frontend.
    ...
    def __init__(self, distro):
        ...
        try:
            self.custom_title = self.db.get('lingmo-installer/custom_title_text')
        ...
        try:
            self.automation_error_cmd = self.db.get(
                'lingmo-installer/automation_failure_command')
            self.error_cmd = self.db.get('lingmo-installer/failure_command')
            self.success_cmd = self.db.get('lingmo-installer/success_command')
        except debconf.DebconfError:
            pass

        try:
            self.show_shutdown_button = \
                self.db.get('lingmo-installer/show_shutdown_button') == 'true'
        except debconf.DebconfError:
            self.show_shutdown_button = False

        self.hide_slideshow = False
        try:
            if self.db.get('lingmo-installer/hide_slideshow') == 'true':
    ...
    def post_mortem(self, exctype, excvalue, exctb):
        ...
        if 'LINGMO-INSTALLER_DEBUG_PDB' not in os.environ:
            return
    ...
    def debconf_communicator(self):
        return DebconfCommunicator('lingmo-installer', cloexec=True)
    ...
    def get_reboot_seen(self):
        ...
        try:
            reboot_seen = self.debconf_operation(
                'fget', 'lingmo-installer/reboot', 'seen')
        except debconf.DebconfError:
            pass
    ...
    def get_shutdown_seen(self):
        ...
        try:
            shutdown_seen = self.debconf_operation(
                'fget', 'lingmo-installer/poweroff', 'seen')
        except debconf.DebconfError:
            pass
    ...
    def run_success_cmd(self):
        if self.success_cmd != '':
            self.debconf_progress_info(
                self.get_string('lingmo-installer/install/success_command'))
