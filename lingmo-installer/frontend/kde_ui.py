...
from lingmo-installer import filteredcommand, i18n, misc, telemetry
from lingmo-installer.components import partman_commit, install, plugininstall
import lingmo-installer.frontend.base
from lingmo-installer.frontend.base import BaseFrontend
from lingmo-installer.frontend.kde_components import ProgressDialog
from lingmo-installer.frontend.kde_components.Breadcrumb import Breadcrumb
from lingmo-installer.frontend.kde_components import qssutils
from lingmo-installer.plugin import Plugin
from lingmo-installer.qtwidgets import SquareSvgWidget
import lingmo-installer.progressposition


# Define global path
PATH = '/usr/share/lingmo-installer'
...
class Lingmo-installerUI(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        # can cause lingmo-installer crashes, because Python's subprocess module
...
class Wizard(BaseFrontend):
    def __init__(self, distro):
        ...
        # Lingmo-installer uses the default Windows-95-like palette when running as a
        # DM.
        ...
        # The "hicolor" icon theme gets picked when Lingmo-installer is running as a
        ...
        self.app.setWindowIcon(QtGui.QIcon.fromTheme("lingmo-installer-kde"))
        import dbus.mainloop.pyqt5
        dbus.mainloop.pyqt5.DBusQtMainLoop(set_as_default=True)

        self.ui = Lingmo-installerUI()
        ...
        logoDirectory = "/usr/share/lingmo-installer/qt/images/"
    ...
    def run(self):
        ...
        if 'LINGMO-INSTALLER_AUTOMATIC' in os.environ:
            ...
            self.progressDialog.setWindowTitle(
                self.get_string('lingmo-installer/install/title'))
            self.refresh()
        ...
        if 'LINGMO-INSTALLER_AUTOMATIC' not in os.environ:
            # Only show now so that the window does not show empty, then resize
            # itself and show content
            self.ui.show()

        if 'LINGMO-INSTALLER_TEST_SLIDESHOW' in os.environ:
            ...
            if self.oem_user_config:
                ...
                if ('LINGMO-INSTALLER_ONLY' in os.environ or
                    ...
                    quitText = self.get_string(
                        'lingmo-installer/finished_restart_only')
                ...
                if ('LINGMO-INSTALLER_ONLY' not in os.environ and
                        'LINGMO-INSTALLER_GREETER' not in os.environ):
                    messageBox.addButton(
    ...
    def _create_webview(self):
        ...
        # LINGMO-INSTALLER_TEST_SLIDESHOW=1 lingmo-installer !). Creating it from the code
    ...
    def start_slideshow(self):
        ...
        slideshow_dir = '/usr/share/lingmo-installer-slideshow'
    ...
    def translate_widget(self, widget, lang=None, prefix=None):
        ...
        if str(name) == "Lingmo-installerUIBase":
        ...
        if isinstance(widget, (QtWidgets.QLabel, Breadcrumb)):
            ...
            # TODO remove small from lingmo-installer.template and disable replace here
            ...
            widget.setText(text.replace('_', '&', 1))
              ...
              str(name) == "Lingmo-installerUIBase"):
            if self.custom_title:
    ...
    def on_quit_clicked(self):
        ...
        yes = self.get_string('yes', prefix='lingmo-installer/imported')
        no = self.get_string('no', prefix='lingmo-installer/imported')
        messageBox = QtWidgets.QMessageBox()
