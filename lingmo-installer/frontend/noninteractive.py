...
class Wizard(BaseFrontend):
    def __init__(self, distro):
        ...
        for mod in self.modules:
            if hasattr(mod.module, 'PageNoninteractive'):
                mod.controller = lingmo-installer.frontend.base.Controller(self)
    ...
    def run(self):
        """Main entry point."""
        # Is this even needed anymore now that Lingmo-installer elevates its
        # privileges?
        if os.getuid() != 0:
            ...
            if ret == 3:
                ...
                sys.exit(ret)
                  ...
                  os.WTERMSIG(ret) in (signal.SIGINT, signal.SIGKILL,
                                       signal.SIGTERM)):
                sys.exit(ret)
            elif os.path.exists('/var/lib/lingmo-installer/install.trace'):
                with open('/var/lib/lingmo-installer/install.trace') as tbfile:
                    realtb = tbfile.read()
    ...
    # lingmo-installer.components.partman_commit
