...
from lingmo-installer import (
    filteredcommand, gsettings, i18n, validation, misc, osextras, telemetry)
from lingmo-installer.components import install, plugininstall, partman_commit
import lingmo-installer.frontend.base
from lingmo-installer.frontend.base import BaseFrontend
from lingmo-installer.plugin import Plugin
import lingmo-installer.progressposition
...
# Define global path
PATH = os.environ.get('LINGMO-INSTALLER_PATH', False) or '/usr/share/lingmo-installer'

# Define ui path
UIDIR = os.environ.get('LINGMO-INSTALLER_GLADE', False) or os.path.join(PATH, 'gtk')
os.environ['LINGMO-INSTALLER_GLADE'] = UIDIR
...
class Controller(lingmo-installer.frontend.base.Controller):
...
def on_screen_reader_enabled_changed(gsettings, key):
    ...
    if key == "screen-reader-enabled":
        ...
        if (gsettings.get_boolean(key) and osextras.find_on_path('orca')):
            ...
            os.environ['LINGMO-INSTALLER_A11Y_PROFILE'] = 'screen-reader'
        elif 'LINGMO-INSTALLER_A11Y_PROFILE' in os.environ:
            # Disable
            del os.environ['LINGMO-INSTALLER_A11Y_PROFILE']
...
class Wizard(BaseFrontend):
    def __init__(self, distro):
        ...
        def add_widget(self, widget):
            ...
            if 'LINGMO-INSTALLER_LDTP' in os.environ:
        ...
        self.progress_position = lingmo-installer.progressposition.ProgressPosition()
        ...
        if 'LINGMO-INSTALLER_ONLY' in os.environ:
            ...
            # handle orca only in lingmo-installer-dm where there is no gnome-session
        ...
        self.builder.add_from_file('%s/lingmo-installer.ui' % UIDIR)
        ...
        next_style.add_class('lingmo-installer-next')
    ...
    def excepthook(self, exctype, excvalue, exctb):
        ...
        if os.path.exists('/usr/share/apport/apport-gtk'):
            ...
            # report; in only-lingmo-installer mode we need to bring up the UI
            # ourselves
            # update-notifier doesn't work on overlayfs so also run if using
            # maybe-lingmo-installer

            # FIXME: Revert the check to maybe-lingmo-installer once inotify on
            # overlayfs is fixed (crash will then be detected by
            # update-notifier)
            with open('/proc/cmdline') as fp:
                if 'lingmo-installer' in fp.read():
            ...
            self.refresh()
            misc.execute_root("apport-bug", "lingmo-installer")
            sys.exit(1)
    ...
    def run(self):
        ...
        if 'LINGMO-INSTALLER_ONLY' in os.environ:
            ...
            if 'LINGMO-INSTALLER_DEBUG' not in os.environ:
        ...
        if 'LINGMO-INSTALLER_AUTOMATIC' in os.environ:
            self.debconf_progress_start(
                0, self.pageslen, self.get_string('lingmo-installer/install/checking'))
        ...
        if self.oem_user_config:
            ...
            self.live_installer.hide()
            if ('LINGMO-INSTALLER_ONLY' in os.environ or
                    'LINGMO-INSTALLER_GREETER' in os.environ):
                txt = self.get_string('lingmo-installer/finished_restart_only')
    ...
    def start_slideshow(self):
        ...
        ltr = i18n.get_string(
            'default-ltr', slideshow_locale, 'lingmo-installer/imported')
        if ltr == 'default:RTL':
        ...
        if (os.environ.get('LINGMO-INSTALLER_A11Y_PROFILE') == 'screen-reader'):
    ...
    def customize_installer(self):
        ...
        self.live_installer.set_icon_name('lingmo-installer')
        ...
        if (self.oem_user_config and
            ...
            self.slideshow = '/usr/share/lingmo-installer-slideshow'
    ...
    def set_window_hints(self, widget):
        if (self.oem_user_config or
                'LINGMO-INSTALLER_ONLY' in os.environ or
                'LINGMO-INSTALLER_GREETER' in os.environ):
    ...
    def lockdown_environment(self):
        ...
        f = (Gdk.WMFunction.RESIZE | Gdk.WMFunction.MAXIMIZE |
             Gdk.WMFunction.MOVE)
        if 'LINGMO-INSTALLER_ONLY' not in os.environ:
    ...
    def translate_reget(self, lang):
        ...
        core_names = ['lingmo-installer/text/%s' % q for q in self.language_questions]
        core_names.append('lingmo-installer/text/oem_config_title')
        core_names.append('lingmo-installer/text/oem_user_config_title')
        core_names.append('lingmo-installer/imported/default-ltr')
        core_names.append('lingmo-installer/text/release_notes_only')
        core_names.append('lingmo-installer/text/update_installer_only')
        core_names.append('lingmo-installer/text/USB')
        core_names.append('lingmo-installer/text/CD')
        stock_items = (
            'cancel', 'close', 'go-back', 'go-forward', 'ok', 'quit')
        for stock_item in stock_items:
            core_names.append('lingmo-installer/imported/%s' % stock_item)
        prefixes = []
        for p in self.pages:
            prefix = p.ui.get('plugin_prefix')
            if not prefix:
                prefix = 'lingmo-installer/text'
    ...
    def translate_widget(self, widget, lang=None, prefix=None):
        ...
        if isinstance(widget, Gtk.Label):
            ...
            if question.startswith('lingmo-installer/imported/'):
    ...
    def set_page(self, n):
        ...
        cur = None
        is_install = False
        if 'LINGMO-INSTALLER_GREETER' in os.environ:
        ...
        if self.pagesindex == 0:
            ...
            # going back.
            self.allow_go_backward(False)
        elif 'LINGMO-INSTALLER_AUTOMATIC' not in os.environ:
    ...
    def do_reboot(self):
        ...
        # don't let reboot race with the shutdown of X in lingmo-installer-dm;
    ...
    def do_shutdown(self):
        ...
        # don't let poweroff race with the shutdown of X in lingmo-installer-dm;
    ...
    def debconffilter_done(self, dbfilter):
        ...
        elif dbfilter.__module__ in ('lingmo-installer.components.install',
                                     'lingmo-installer.components.plugininstall'):
    ...
    def maybe_start_installing(self):
        ...
        if use_zfs == 'true':
            env = os.environ.copy()
            zfs_keystore_key = self.db.get('lingmo-installer/zfs_keystore_key')
            if zfs_keystore_key:
                os.environ['ZFS_KS_KEY'] = zfs_keystore_key
            misc.execute_root('/usr/share/lingmo-installer/zsys-setup', 'init')
            os.environ.clear()
            os.environ.update(env)

        syslog.syslog('Starting the installation')

        from lingmo-installer.debconfcommunicator import DebconfCommunicator
        ...
        self.parallel_db = DebconfCommunicator('lingmo-installer',
                                               cloexec=True,
                                               env=env)
    ...
    def find_next_step(self, finished_step):
        ...
        if finished_step == last_page and not self.backup:
            ...
            env['DEBCONF_DB_REPLACE'] = 'configdb'
            env['DEBCONF_DB_OVERRIDE'] = 'Pipe{infd:none outfd:none}'
            self.parallel_db = DebconfCommunicator('lingmo-installer', cloexec=True,
                                                   env=env)
        ...
        elif finished_step == 'lingmo-installer.components.partman_commit':
            self.partitioned = True
            self.maybe_start_installing()

        # FIXME OH DEAR LORD.  Use isinstance.
        elif finished_step == 'lingmo-installer.components.install':
        ...
        elif finished_step == 'lingmo-installer.components.plugininstall':
            self.installing = False
