...
from lingmo-installer import gsettings, keyboard_names, misc, osextras, plugin
...
class PageGtk(plugin.PluginUI):
    plugin_title = 'lingmo-installer/text/keyboard_heading_label'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        try:
            ...
            builder.add_from_file(os.path.join(
                os.environ['LINGMO-INSTALLER_GLADE'], 'stepKeyboardConf.ui'))
    ...
    def calculate_clicked(self, *args):
        from lingmo-installer.frontend.gtk_components.keyboard_query import (
            KeyboardQuery,
        )
...
class PageKde(plugin.PluginUI):
    plugin_breadcrumb = 'lingmo-installer/text/breadcrumb_keyboard'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        try:
            ...
            from lingmo-installer.frontend.kde_components.Keyboard import Keyboard

            self.page = uic.loadUi(
                '/usr/share/lingmo-installer/qt/stepKeyboardConf.ui')
...
class Page(plugin.Plugin):
    def prepare(self, unfiltered=False):
        ...
        with misc.raised_privileges():
            osextras.unlink_force('/etc/default/keyboard.pre-lingmo-installer')
            try:
                os.rename('/etc/default/keyboard',
                          '/etc/default/keyboard.pre-lingmo-installer')
            except OSError:
        ...
        # bother for now.
        command = [
            '/usr/lib/lingmo-installer/console-setup/keyboard-configuration.postinst',
            'configure',
        ]
        questions = [
        ...
        environ = {
            ...
            'LC_ALL': di_locale,
            'PATH': '/usr/lib/lingmo-installer/console-setup:' + os.environ['PATH'],
    ...
    def run(self, priority, question):
        ...
        if question == 'keyboard-configuration/layout':
            ...
            response = self.frontend.question_dialog(
                ...
                ('lingmo-installer/imported/yes', 'lingmo-installer/imported/no'))
            if response == 'lingmo-installer/imported/yes':
...
class Install(plugin.InstallPlugin):
    def prepare(self, unfiltered=False):
        return (['/usr/share/lingmo-installer/console-setup-apply'], [])

    def install(self, target, progress, *args, **kwargs):
        progress.info('lingmo-installer/install/keyboard')
