...
from lingmo-installer import i18n, misc, osextras, plugin, upower
from lingmo-installer.install_misc import (archdetect, is_secure_boot,
...
class PreparePageBase(plugin.PluginUI):
    plugin_title = 'lingmo-installer/text/prepare_heading_label'
...
class PageGtk(PreparePageBase):
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepPrepare.ui'))
    ...
    def show_rst_page(self):
        for page in self.controller._wizard.pages:
            if page.module.NAME == NAME:
                page.title = 'lingmo-installer/text/rst_header'
                break
    ...
    def enable_download_updates(self, val):
        if (val):
            template = 'lingmo-installer/text/label_download_updates'
        else:
            template = 'lingmo-installer/text/label_download_updates_na'
    ...
    def plugin_translate(self, lang):
        ...
        frontend.status_label.set_text(self.controller.get_string(
            'lingmo-installer/text/preparing_ud_label', lang))
    ...
    def show_learn_more(self, unused):
        ...
        sb_title_template = 'lingmo-installer/text/efi_secureboot'
        sb_info_template = 'lingmo-installer/text/efi_secureboot_info'
...
class PageKde(PreparePageBase):
    plugin_breadcrumb = 'lingmo-installer/text/breadcrumb_prepare'
    restricted_package_name = 'klingmo-restricted-addons'

    def __init__(self, controller, *args, **kwargs):
        from lingmo-installer.qtwidgets import StateBox
        self.controller = controller
        try:
            ...
            from lingmo-installer.frontend.kde_components.nmwidgets import ProgressIndicator
            self.page = uic.loadUi('/usr/share/lingmo-installer/qt/stepPrepare.ui')
...
class Page(plugin.Plugin):
    def prepare(self):
        if (self.db.get('apt-setup/restricted') == 'false' or
            ...
            use_nonfree = self.db.get('lingmo-installer/use_nonfree') == 'true'
        ...
        self.ui.download_updates = self.db.get('lingmo-installer/download_updates') == 'true'
        if self.ui.download_updates_enabled:
            self.ui.set_download_updates(self.ui.download_updates)
        minimal_install = self.db.get('lingmo-installer/minimal_install') == 'true'
        ...
        if self.should_show_rst_page():
            ...
            self.setup_sufficient_space()
        command = ['/usr/share/lingmo-installer/simple-plugins', 'prepare']
        questions = ['lingmo-installer/use_nonfree']
        return command, questions

    def apply_debconf_branding(self):
        release = misc.get_release()
        for template in ['lingmo-installer/text/required_space',
                         'lingmo-installer/text/free_space']:
    ...
    def setup_sufficient_space(self):
        ...
        self.db.subst(
            'lingmo-installer/text/required_space', 'SIZE',
            misc.format_size(size))
        free = self.free_space()
        self.db.subst(
            'lingmo-installer/text/free_space', 'SIZE',
            misc.format_size(free))
        required_text = self.description('lingmo-installer/text/required_space')
        free_text = self.description('lingmo-installer/text/free_space')
    ...
    def ok_handler(self):
        ...
        self.preseed_bool('lingmo-installer/use_nonfree', use_nonfree)
        self.preseed_bool('lingmo-installer/download_updates', download_updates)
        self.preseed_bool('lingmo-installer/minimal_install', minimal_install)
        if self.ui.using_secureboot and secureboot_key:
            self.preseed('lingmo-installer/secureboot_key', secureboot_key, seen=True)
        if use_nonfree:
            with misc.raised_privileges():
                # Install lingmo-restricted-addons.
                self.preseed_bool('apt-setup/universe', True)
                self.preseed_bool('apt-setup/multiverse', True)
                if self.db.fget('lingmo-installer/nonfree_package', 'seen') != 'true':
                    self.preseed(
                        'lingmo-installer/nonfree_package',
