...
from lingmo-installer import i18n, misc, plugin
import lingmo-installer.tz
...
_geoname_url = 'https://geoname-lookup.lingmo.com/?query=%s&release=%s'


class PageGtk(plugin.PluginUI):
    plugin_title = 'lingmo-installer/text/timezone_heading_label'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepLocation.ui'))
    ...
    def plugin_translate(self, lang):
        # c = self.controller
        # if c.get_string('lingmo-installer/imported/time-format', lang) == '12-hour':
        #    fmt = c.get_string('lingmo-installer/imported/12-hour', lang)
        # else:
        #    fmt = c.get_string('lingmo-installer/imported/24-hour', lang)
    ...
    def changed(self, entry):
        ...
        if text in self.geoname_cache:
            ...
            message.request_headers.append('User-agent', 'Lingmo-installer/1.0')
    ...
    def setup_page(self):
        ...
        self.tzdb = lingmo-installer.tz.Database()
...
class PageKde(plugin.PluginUI):
    plugin_breadcrumb = 'lingmo-installer/text/breadcrumb_timezone'

    def __init__(self, controller, *args, **kwargs):
        self.controller = controller
        try:
            from PyQt5 import uic
            from lingmo-installer.frontend.kde_components.Timezone import TimezoneMap

            self.page = uic.loadUi('/usr/share/lingmo-installer/qt/stepLocation.ui')
...
class Page(plugin.Plugin):
    def prepare(self, unfiltered=False):
        ...
        clock_script = '/usr/share/lingmo-installer/clock-setup'
        env = {'PATH': '/usr/share/lingmo-installer:' + os.environ['PATH']}

        # TODO: replace with more general version pushed down into
        # is_automatic or similar
        try:
            self.automatic_page = (
                self.db.get("lingmo-installer/automatic/timezone") == "true")
...
class Install(plugin.InstallPlugin):
    def prepare(self, unfiltered=False):
        tzsetup_script = '/usr/lib/lingmo-installer/tzsetup/post-base-installer'
        clock_script = '/usr/share/lingmo-installer/clock-setup-apply'
    ...
    def install(self, target, progress, *args, **kwargs):
        progress.info('lingmo-installer/install/timezone')
