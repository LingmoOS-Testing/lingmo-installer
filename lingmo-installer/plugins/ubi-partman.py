...
from lingmo-installer import (misc, osextras, parted_server, plugin,
                      telemetry, validation)
from lingmo-installer.install_misc import archdetect
...
# https://bugs.launchpad.net/lingmo/+source/lingmo-installer/+bug/959724
...
class PageGtk(PageBase):
    plugin_title = 'lingmo-installer/text/part_auto_heading_label'
    plugin_is_install = True

    def __init__(self, controller, *args, **kwargs):
        self.controller = controller
        from gi.repository import Gtk
        from lingmo-installer.gtkwidgets import Builder
        builder = Builder()
        self.controller.add_builder(builder)
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepPartAsk.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepPartAuto.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepPartAdvanced.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], 'stepPartCrypto.ui'))
    ...
    def plugin_on_next_clicked(self):
        ...
        if resize and self.should_show_bitlocker_page():
            title = self.controller.get_string('lingmo-installer/text/bitlocker_header')
        ...
        if recovery_key_location_path:
            try:
                ...
                save_recovery_key_info = 'lingmo-installer/text/save_recovery_key_info'
    ...
    def initialize_resize_mode(self):
        ...
        # same install_size as everywhere else in lingmo-installer
        ...
        if not title:
            ...
            title = self.controller.get_string('lingmo-installer/text/part_auto_files')
            title = title.replace('${SIZE}', misc.format_size(resize_min_size))
            icon.set_property('icon-name', 'folder')
        else:
            if 'windows' in title.lower():
                PATH = (
                    os.environ.get('LINGMO-INSTALLER_PATH', False) or
                    '/usr/share/lingmo-installer')
    ...
    def set_autopartition_options(self, options, extra_options):
        ...
        if 'some_device_crypto' in extra_options:
            title = self.controller.get_string(
                'lingmo-installer/text/use_crypto')
            title = title.replace('${RELEASE}', release.name)
            desc = self.controller.get_string('lingmo-installer/text/use_crypto_desc')
            options['some_device_crypto'] = PartitioningOption(title, desc)

        if 'some_device_lvm' in extra_options:
            title = self.controller.get_string('lingmo-installer/text/use_lvm')
            title = title.replace('${RELEASE}', release.name)
            desc = self.controller.get_string('lingmo-installer/text/use_lvm_desc')
    ...
    def update_partman(self, disk_cache, partition_cache, cache_order):
        from gi.repository import Gtk, GObject
        from lingmo-installer import segmented_bar
    ...
    def show_crypto_page(self):
        self.set_page_title(
            self.controller.get_string('lingmo-installer/text/crypto_label'))
...
class PageKde(PageBase):
    plugin_breadcrumb = 'lingmo-installer/text/breadcrumb_partition'
    plugin_is_install = True
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        from lingmo-installer.frontend.kde_components.PartAuto import PartAuto
        from lingmo-installer.frontend.kde_components.PartMan import PartMan
...
class Page(plugin.Plugin):
    def prepare(self):
        ...
        if (self.db.get('lingmo-installer/install_bootloader') == 'true' and
                'LINGMO-INSTALLER_NO_BOOTLOADER' not in os.environ):
        ...
        command = [
            'sh', '-c',
            '/usr/share/lingmo-installer/activate-dmraid && /bin/partman',
        ]
        questions = [
    ...
    def set(self, question, value):
        if question == 'lingmo-installer/partman-rebuild-cache':
    ...
    def calculate_reuse_option(self):
        '''Takes the current Lingmo version on disk and the release we're about
        to install as parameters.'''

        # TODO: verify that lingmo is the same partition as one of the ones
        #       offered in the reuse options.
        release = misc.get_release()
        if 'reuse' in self.extra_options:
            reuse = self.extra_options['reuse']
            if len(reuse) == 1:
                lingmo, current_version = misc.find_in_os_prober(
                    reuse[0][1], with_version=True)
                final = current_version in lingmo
                try:
                ...
                if current_version == new_version and final:
                    # "Windows (or Mac, ...) and the current version of Lingmo
                    # are present" case
                    q = 'lingmo-installer/partitioner/lingmo_reinstall'
                    self.db.subst(q, 'CURDISTRO', lingmo)
    ...
    def calculate_autopartitioning_heading(self, operating_systems,
        ...
        if os_count == 1:
            q = 'lingmo-installer/partitioner/heading_one'
            self.db.subst(q, 'OS', operating_systems[0])
            return self.extended_description(q)
        elif os_count == 2 and has_lingmo:
            q = 'lingmo-installer/partitioner/heading_dual'
            self.db.subst(q, 'OS1', operating_systems[0])
            self.db.subst(q, 'OS2', operating_systems[1])
            return self.extended_description(q)
        else:
            q = 'lingmo-installer/partitioner/heading_multiple'
            return self.extended_description(q)
    ...
    def calculate_operating_systems(self, layout):
        ...
        lingmo_systems = [x for x in operating_systems
                          if x.lower().find('buntu') != -1]
        return (operating_systems, lingmo_systems)
    ...
    def calculate_autopartitioning_options(self, operating_systems,
        ...
        - An older version of Lingmo is present
        - There are no operating systems present
        - Windows (or Mac, ...) and an older version of Lingmo are present
        - Windows (or Mac, ...) and the current version of Lingmo are present
        ...
        q = 'lingmo-installer/partitioner/advanced'
        ...
        if os_count == 0:
            ...
            q = 'lingmo-installer/partitioner/multiple_os_format'
            self.db.subst(q, 'DISTRO', release.name)
            title = self.description(q)
            desc = self.extended_description(q)
            ...
            if len(lingmo_systems) == 1:
                # "An older version of Lingmo is present" case
                if 'replace' in self.extra_options:
                    q = 'lingmo-installer/partitioner/lingmo_format'
                ...
                if wubi_option:
                    # We don't have a Wubi-like solution for Lingmo yet (though
                    # wubi_option is also a check for ntfs).
                    pass
                elif resize_option:
                    q = 'lingmo-installer/partitioner/lingmo_resize'
                    self.db.subst(q, 'DISTRO', release.name)
                ...
                if wubi_option or resize_option or bitlocker:
                    if resize_option or bitlocker:
                        q = 'lingmo-installer/partitioner/single_os_resize'
                    else:
                        q = 'lingmo-installer/partitioner/lingmo_inside'
        ...
        elif os_count == 2 and len(lingmo_systems) == 1:
            # TODO: verify that lingmo_systems[0] is the same partition as one
            # of the ones offered in the replace options.
            lingmo = lingmo_systems[0]
            if 'replace' in self.extra_options:
                q = 'lingmo-installer/partitioner/lingmo_format'
                self.db.subst(q, 'CURDISTRO', lingmo)
                title = self.description(q)
            ...
            if wubi_option:
                ...
                q = 'lingmo-installer/partitioner/multiple_os_resize'
                self.db.subst(q, 'DISTRO', release.name)
                title = self.description(q)
                desc = self.extended_description(q)
                opt = PartitioningOption(title, desc)
    ...
    def run(self, priority, question):
        ...
        if question.endswith('automatically_partition'):
            ...
            operating_systems, lingmo_systems = \
                self.calculate_operating_systems(layout)
            has_lingmo = len(lingmo_systems) > 0
            heading = self.calculate_autopartitioning_heading(
                operating_systems, has_lingmo)
            ...
            if 'manual' not in self.extra_options:
                ...
                # Here be dragons...
                self.preseed('lingmo-installer/partman-skip-unmount', 'true',
                             seen=False)
            ...
            if self.creating_label:
                response = self.frontend.question_dialog(
                    ...
                    self.extended_description(question),
                    ('lingmo-installer/text/go_back', 'lingmo-installer/text/continue'))
                if response == 'lingmo-installer/text/continue':
            ...
            if do_preseed:
                ...
                self.preseed('lingmo-installer/crypto_key', self.ui.get_crypto_keys())
                self.preseed('lingmo-installer/recovery_key', self.ui.get_recovery_keys())
                return True
            ...
            response = self.frontend.question_dialog(
                ...
                ('lingmo-installer/text/go_back', 'lingmo-installer/text/continue'))
            if response == 'lingmo-installer/text/continue':
                self.db.set('lingmo-installer/partman-confirm', question[8:])
                self.preseed(question, 'true', seen=False)
                self.succeeded = True
                self.done = True
            else:
            ...
            if question == 'partman/unmount_active':
                yes = 'lingmo-installer/imported/yes'
                no = 'lingmo-installer/imported/no'
            elif question == 'partman-efi/non_efi_system':
                yes = 'lingmo-installer/text/in_uefi_mode'
                no = 'lingmo-installer/text/go_back'
            else:
                yes = 'lingmo-installer/text/continue'
                no = 'lingmo-installer/text/go_back'
            response = self.frontend.question_dialog(
                self.description(question),
    ...
    def update_zfs_description(self, description):
        ...
        misc.execute_root('/usr/share/lingmo-installer/zsys-setup', 'layout', device)
        ...
        partlabel = misc.utf8(self.db.metaget('lingmo-installer/text/partman_confirm_zfs', 'extended_description'),
                              errors='replace')
        for line in layout:
    ...
    def ok_handler(self):
        ...
        if self.current_question.endswith('automatically_partition'):
            ...
            self.db.set('lingmo-installer/use_zfs',
            ...
            if keystore_key:
                self.db.set('lingmo-installer/zfs_keystore_key', keystore_key)
