...
from lingmo-installer import auto_update, i18n, misc, osextras, plugin
...
try:
    ...
    _ver = '12.04'
_wget_url = 'http://changelogs.lingmo.com/lingmo-installer/%s-update-available' % _ver
...
class PageGtk(PageBase):
    ...
    plugin_title = 'lingmo-installer/text/language_heading_label'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['LINGMO-INSTALLER_GLADE'], ui_file))
        ...
        if not self.only:
            if 'LINGMO-INSTALLER_GREETER' not in os.environ:
    ...
    def set_language(self, language):
        ...
        if not self.only and 'LINGMO-INSTALLER_GREETER' in os.environ:
    ...
    def on_language_selection_changed(self, *args, **kwargs):
        ...
        from gi.repository import Gtk
        ltr = i18n.get_string('default-ltr', lang, 'lingmo-installer/imported')
        if ltr == 'default:RTL':
...
class PageKde(PageBase):
    plugin_breadcrumb = 'lingmo-installer/text/breadcrumb_language'
    plugin_is_language = True
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        try:
            ...
            self.page = uic.loadUi('/usr/share/lingmo-installer/qt/stepLanguage.ui')
            ...
            def init_big_button(button, image_name):
                pix = QPixmap('/usr/share/lingmo-installer/qt/images/' + image_name)
                icon = QIcon(pix)
                button.setIcon(icon)
...
class Page(plugin.Plugin):
    def prepare(self, unfiltered=False):
        ...
        localechooser_script = '/usr/lib/lingmo-installer/localechooser/localechooser'
        if ('LINGMO-INSTALLER_FRONTEND' in os.environ and
                os.environ['LINGMO-INSTALLER_FRONTEND'] == 'debconf_ui'):
            localechooser_script += '-debconf'

        questions = ['localechooser/languagelist']
        environ = {
            'PATH': '/usr/lib/lingmo-installer/localechooser:' + os.environ['PATH'],
        }
        if ('LINGMO-INSTALLER_FRONTEND' in os.environ and
                os.environ['LINGMO-INSTALLER_FRONTEND'] == "debconf_ui"):
    ...
    def run(self, priority, question):
        if question == 'localechooser/languagelist':
            ...
            only_installable = misc.create_bool(
                self.db.get('lingmo-installer/only-show-installable-languages'))
...
class Install(plugin.InstallPlugin):
    def prepare(self, unfiltered=False):
        if 'LINGMO-INSTALLER_OEM_USER_CONFIG' in os.environ:
            command = ['/usr/share/lingmo-installer/localechooser-apply']
        else:
            command = [
                'sh', '-c',
                '/usr/lib/lingmo-installer/localechooser/post-base-installer ' +
                '&& /usr/lib/lingmo-installer/localechooser/finish-install',
            ]
        return command, []
    ...
    def install(self, target, progress, *args, **kwargs):
        progress.info('lingmo-installer/install/locales')
