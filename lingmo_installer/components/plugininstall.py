...
class Install(FilteredCommand):
    def prepare(self, unfiltered=False):
        ...
        install_bootloader_seen = self.db.fget('lingmo_installer/install_bootloader',
                                               'seen')
        if not (automatic_mode and install_bootloader_seen):
            if self.frontend.oem_user_config:
                self.preseed_bool('lingmo_installer/install_bootloader', False)
            else:
                self.preseed_bool('lingmo_installer/install_bootloader', True)
        ...
        questions = ['^.*/apt-install-failed$',
                     ...
                     'ERROR',
                     'PROGRESS']
        return (['/usr/share/lingmo_installer/plugininstall.py'], questions)
    ...
    def error(self, priority, question):
        if question == 'hw-detect/modprobe_error':
            ...
            fatal = False
        elif (question == 'lingmo_installer/install/broken_active_directory' or
              question == 'lingmo_installer/install/broken_luks_add_key'):
            fatal = False
        else:
            fatal = True
    ...
    def run(self, priority, question):
        ...
        elif question == 'lingmo_installer/install/new-bootdev':
