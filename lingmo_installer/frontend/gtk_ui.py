...
# - Evan Dandrea <ev@lingmo.com>
# - Mario Limonciello <superm1@lingmo.com>
#
# This file is part of Lingmo_installer.
#
# Lingmo_installer is free software; you can redistribute it and/or modify it under
# the terms of the GNU General Public License as published by the Free
# Software Foundation; either version 2 of the License, or at your option)
# any later version.
#
# Lingmo_installer is distributed in the hope that it will be useful, but WITHOUT
# ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
# FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for
# more details.
#
# You should have received a copy of the GNU General Public License along
# with Lingmo_installer; if not, write to the Free Software Foundation, Inc., 51
...
from lingmo_installer import (
    filteredcommand, gsettings, i18n, validation, misc, osextras, telemetry)
from lingmo_installer.components import install, plugininstall, partman_commit
import lingmo_installer.frontend.base
from lingmo_installer.frontend.base import BaseFrontend
from lingmo_installer.plugin import Plugin
import lingmo_installer.progressposition
...
# Define global path
PATH = os.environ.get('LINGMO_INSTALLER_PATH', False) or '/usr/share/lingmo_installer'

# Define ui path
UIDIR = os.environ.get('LINGMO_INSTALLER_GLADE', False) or os.path.join(PATH, 'gtk')
os.environ['LINGMO_INSTALLER_GLADE'] = UIDIR
...
class Controller(lingmo_installer.frontend.base.Controller):
...
def on_screen_reader_enabled_changed(gsettings, key):
    ...
    if key == "screen-reader-enabled":
        ...
        if (gsettings.get_boolean(key) and osextras.find_on_path('orca')):
            ...
            os.environ['LINGMO_INSTALLER_A11Y_PROFILE'] = 'screen-reader'
        elif 'LINGMO_INSTALLER_A11Y_PROFILE' in os.environ:
            # Disable
            del os.environ['LINGMO_INSTALLER_A11Y_PROFILE']
...
class Wizard(BaseFrontend):
    def __init__(self, distro):
        ...
        def add_widget(self, widget):
            ...
            if 'LINGMO_INSTALLER_LDTP' in os.environ:
        ...
        self.progress_position = lingmo_installer.progressposition.ProgressPosition()
        self.progress_cancelled = False
        self.installing = False
        self.installing_no_return = False
        self.partitioned = False
        self.timezone_set = False
        self.lingmo_drivers = None
        self.returncode = 0
        self.history = []
        ...
        if 'LINGMO_INSTALLER_ONLY' in os.environ:
            ...
            # handle orca only in lingmo_installer-dm where there is no gnome-session
        ...
        self.builder.add_from_file('%s/lingmo_installer.ui' % UIDIR)
        ...
        next_style.add_class('lingmo_installer-next')
    ...
    def save_oem_metapackages_list(self, wait_finished=False):
        ''' If we can, update the apt indexes. Then run 'lingmo-drivers
        ...
        with misc.raised_privileges():
            try:
                ...
                syslog.syslog(syslog.LOG_INFO, "Trying to update apt indexes "
                              "to run lingmo-drivers against fresh data.")
            ...
            if osextras.find_on_path('lingmo-drivers'):
                ...
                # point running again.
                if os.path.exists('/run/lingmo-drivers-oem.autoinstall') and \
                        not self.have_apt_updated:
                    return

                # It's either the first run or a subsequent one but we now
                # managed to update the apt indexes.
                self.lingmo_drivers = subprocess.Popen(
                    ['lingmo-drivers',
                     'list-oem',
                     '--package-list',
                     '/run/lingmo-drivers-oem.autoinstall'],
                    stdin=subprocess.DEVNULL,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.DEVNULL)

                if wait_finished:
                    self.lingmo_drivers.communicate()
                    self.lingmo_drivers = None
    ...
    def excepthook(self, exctype, excvalue, exctb):
        ...
        if os.path.exists('/usr/share/apport/apport-gtk'):
            ...
            # report; in only-lingmo_installer mode we need to bring up the UI
            # ourselves
            # update-notifier doesn't work on overlayfs so also run if using
            # maybe-lingmo_installer

            # FIXME: Revert the check to maybe-lingmo_installer once inotify on
            # overlayfs is fixed (crash will then be detected by
            # update-notifier)
            with open('/proc/cmdline') as fp:
                if 'lingmo_installer' in fp.read():
            ...
            self.refresh()
            misc.execute_root("apport-bug", "lingmo_installer")
            sys.exit(1)
    ...
    def run(self):
        ...
        if 'LINGMO_INSTALLER_ONLY' in os.environ:
            ...
            if 'LINGMO_INSTALLER_DEBUG' not in os.environ:
        ...
        if 'LINGMO_INSTALLER_AUTOMATIC' in os.environ:
            self.debconf_progress_start(
                0, self.pageslen, self.get_string('lingmo_installer/install/checking'))
        ...
        if self.oem_user_config:
            ...
            self.live_installer.hide()
            if ('LINGMO_INSTALLER_ONLY' in os.environ or
                    'LINGMO_INSTALLER_GREETER' in os.environ):
                txt = self.get_string('lingmo_installer/finished_restart_only')
    ...
    def start_slideshow(self):
        ...
        ltr = i18n.get_string(
            'default-ltr', slideshow_locale, 'lingmo_installer/imported')
        if ltr == 'default:RTL':
        ...
        if (os.environ.get('LINGMO_INSTALLER_A11Y_PROFILE') == 'screen-reader'):
    ...
    def customize_installer(self):
        ...
        self.live_installer.set_icon_name('lingmo_installer')
        ...
        fontdesc = Pango.font_description_from_string("Lingmo Mono 8")
        ...
        if (self.oem_user_config and
            ...
            self.slideshow = '/usr/share/lingmo_installer-slideshow'
    ...
    def set_window_hints(self, widget):
        if (self.oem_user_config or
                'LINGMO_INSTALLER_ONLY' in os.environ or
                'LINGMO_INSTALLER_GREETER' in os.environ):
    ...
    def lockdown_environment(self):
        ...
        f = (Gdk.WMFunction.RESIZE | Gdk.WMFunction.MAXIMIZE |
             Gdk.WMFunction.MOVE)
        if 'LINGMO_INSTALLER_ONLY' not in os.environ:
    ...
    def translate_reget(self, lang):
        ...
        core_names = ['lingmo_installer/text/%s' % q for q in self.language_questions]
        core_names.append('lingmo_installer/text/oem_config_title')
        core_names.append('lingmo_installer/text/oem_user_config_title')
        core_names.append('lingmo_installer/imported/default-ltr')
        core_names.append('lingmo_installer/text/release_notes_only')
        core_names.append('lingmo_installer/text/update_installer_only')
        core_names.append('lingmo_installer/text/USB')
        core_names.append('lingmo_installer/text/CD')
        stock_items = (
            'cancel', 'close', 'go-back', 'go-forward', 'ok', 'quit')
        for stock_item in stock_items:
            core_names.append('lingmo_installer/imported/%s' % stock_item)
        prefixes = []
        for p in self.pages:
            prefix = p.ui.get('plugin_prefix')
            if not prefix:
                prefix = 'lingmo_installer/text'
    ...
    def translate_widget(self, widget, lang=None, prefix=None):
        ...
        if isinstance(widget, Gtk.Label):
            ...
            if question.startswith('lingmo_installer/imported/'):
    ...
    def set_page(self, n):
        ...
        cur = None
        is_install = False
        if 'LINGMO_INSTALLER_GREETER' in os.environ:
        ...
        if self.pagesindex == 0:
            ...
            # going back.
            self.allow_go_backward(False)
        elif 'LINGMO_INSTALLER_AUTOMATIC' not in os.environ:
    ...
    def do_reboot(self):
        ...
        # don't let reboot race with the shutdown of X in lingmo_installer-dm;
    ...
    def do_shutdown(self):
        ...
        # don't let poweroff race with the shutdown of X in lingmo_installer-dm;
    ...
    def debconffilter_done(self, dbfilter):
        ...
        elif dbfilter.__module__ in ('lingmo_installer.components.install',
                                     'lingmo_installer.components.plugininstall'):
    ...
    def maybe_start_installing(self):
        ...
        if use_zfs == 'true':
            env = os.environ.copy()
            zfs_keystore_key = self.db.get('lingmo_installer/zfs_keystore_key')
            if zfs_keystore_key:
                os.environ['ZFS_KS_KEY'] = zfs_keystore_key
            misc.execute_root('/usr/share/lingmo_installer/zsys-setup', 'init')
            os.environ.clear()
            os.environ.update(env)

        syslog.syslog('Starting the installation')

        from lingmo_installer.debconfcommunicator import DebconfCommunicator
        ...
        self.parallel_db = DebconfCommunicator('lingmo_installer',
                                               cloexec=True,
                                               env=env)
    ...
    def find_next_step(self, finished_step):
        ...
        if finished_step == last_page and not self.backup:
            ...
            env['DEBCONF_DB_REPLACE'] = 'configdb'
            env['DEBCONF_DB_OVERRIDE'] = 'Pipe{infd:none outfd:none}'
            self.parallel_db = DebconfCommunicator('lingmo_installer', cloexec=True,
                                                   env=env)
        ...
        elif finished_step == 'lingmo_installer.components.partman_commit':
            self.partitioned = True
            self.maybe_start_installing()

        # FIXME OH DEAR LORD.  Use isinstance.
        elif finished_step == 'lingmo_installer.components.install':
        ...
        elif finished_step == 'lingmo_installer.components.plugininstall':
            self.installing = False
