...
#
# Author(s):
#   Jonathan Riddell <jriddell@lingmo.com>
...
from lingmo_installer import filteredcommand, i18n, misc, telemetry
from lingmo_installer.components import partman_commit, install, plugininstall
import lingmo_installer.frontend.base
from lingmo_installer.frontend.base import BaseFrontend
from lingmo_installer.frontend.kde_components import ProgressDialog
from lingmo_installer.frontend.kde_components.Breadcrumb import Breadcrumb
from lingmo_installer.frontend.kde_components import qssutils
from lingmo_installer.plugin import Plugin
from lingmo_installer.qtwidgets import SquareSvgWidget
import lingmo_installer.progressposition


# Define global path
PATH = '/usr/share/lingmo_installer'
...
class Lingmo_installerUI(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        # can cause lingmo_installer crashes, because Python's subprocess module
...
class Wizard(BaseFrontend):
    def __init__(self, distro):
        ...
        # Lingmo_installer uses the default Windows-95-like palette when running as a
        # DM.
        ...
        # The "hicolor" icon theme gets picked when Lingmo_installer is running as a
        ...
        self.app.setWindowIcon(QtGui.QIcon.fromTheme("lingmo_installer-kde"))
        import dbus.mainloop.pyqt5
        dbus.mainloop.pyqt5.DBusQtMainLoop(set_as_default=True)

        self.ui = Lingmo_installerUI()
        ...
        logoDirectory = "/usr/share/lingmo_installer/qt/images/"
    ...
    def run(self):
        ...
        if 'LINGMO_INSTALLER_AUTOMATIC' in os.environ:
            ...
            self.progressDialog.setWindowTitle(
                self.get_string('lingmo_installer/install/title'))
            self.refresh()
        ...
        if 'LINGMO_INSTALLER_AUTOMATIC' not in os.environ:
            # Only show now so that the window does not show empty, then resize
            # itself and show content
            self.ui.show()

        if 'LINGMO_INSTALLER_TEST_SLIDESHOW' in os.environ:
            ...
            if self.oem_user_config:
                ...
                if ('LINGMO_INSTALLER_ONLY' in os.environ or
                    ...
                    quitText = self.get_string(
                        'lingmo_installer/finished_restart_only')
                ...
                if ('LINGMO_INSTALLER_ONLY' not in os.environ and
                        'LINGMO_INSTALLER_GREETER' not in os.environ):
                    messageBox.addButton(
    ...
    def _create_webview(self):
        ...
        # LINGMO_INSTALLER_TEST_SLIDESHOW=1 lingmo_installer !). Creating it from the code
    ...
    def start_slideshow(self):
        ...
        slideshow_dir = '/usr/share/lingmo_installer-slideshow'
    ...
    def translate_widget(self, widget, lang=None, prefix=None):
        ...
        if str(name) == "Lingmo_installerUIBase":
        ...
        if isinstance(widget, (QtWidgets.QLabel, Breadcrumb)):
            ...
            # TODO remove small from lingmo_installer.template and disable replace here
            ...
            widget.setText(text.replace('_', '&', 1))
              ...
              str(name) == "Lingmo_installerUIBase"):
            if self.custom_title:
    ...
    def on_quit_clicked(self):
        ...
        yes = self.get_string('yes', prefix='lingmo_installer/imported')
        no = self.get_string('no', prefix='lingmo_installer/imported')
        messageBox = QtWidgets.QMessageBox()
