...
from lingmo_installer import (misc, osextras, parted_server, plugin,
                              telemetry, validation)
from lingmo_installer.install_misc import archdetect
...
# https://bugs.launchpad.net/lingmo/+source/lingmo_installer/+bug/959724
...
class PageGtk(PageBase):
    plugin_title = 'lingmo_installer/text/part_auto_heading_label'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['LINGMO_INSTALLER_GLADE'], 'stepPartAsk.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO_INSTALLER_GLADE'], 'stepPartAuto.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO_INSTALLER_GLADE'], 'stepPartAdvanced.ui'))
        builder.add_from_file(os.path.join(
            os.environ['LINGMO_INSTALLER_GLADE'], 'stepPartCrypto.ui'))
    ...
    def plugin_on_next_clicked(self):
        ...
        if resize and self.should_show_bitlocker_page():
            title = self.controller.get_string('lingmo_installer/text/bitlocker_header')
        ...
        if recovery_key_location_path:
            try:
                ...
                save_recovery_key_info = 'lingmo_installer/text/save_recovery_key_info'
    ...
    def initialize_resize_mode(self):
        ...
        # same install_size as everywhere else in lingmo_installer
        ...
        if not title:
            ...
            title = self.controller.get_string('lingmo_installer/text/part_auto_files')
            title = title.replace('${SIZE}', misc.format_size(resize_min_size))
            icon.set_property('icon-name', 'folder')
        else:
            if 'windows' in title.lower():
                PATH = (
                    os.environ.get('LINGMO_INSTALLER_PATH', False) or
                    '/usr/share/lingmo_installer')
    ...
    def set_autopartition_options(self, options, extra_options):
        ...
        if 'some_device_crypto' in extra_options:
            title = self.controller.get_string(
                'lingmo_installer/text/use_crypto')
            title = title.replace('${RELEASE}', release.name)
            desc = self.controller.get_string('lingmo_installer/text/use_crypto_desc')
            options['some_device_crypto'] = PartitioningOption(title, desc)

        if 'some_device_lvm' in extra_options:
            title = self.controller.get_string('lingmo_installer/text/use_lvm')
            title = title.replace('${RELEASE}', release.name)
            desc = self.controller.get_string('lingmo_installer/text/use_lvm_desc')
    ...
    def update_partman(self, disk_cache, partition_cache, cache_order):
        from gi.repository import Gtk, GObject
        from lingmo_installer import segmented_bar
    ...
    def show_crypto_page(self):
        self.set_page_title(
            self.controller.get_string('lingmo_installer/text/crypto_label'))
...
class PageKde(PageBase):
    plugin_breadcrumb = 'lingmo_installer/text/breadcrumb_partition'
    plugin_is_install = True
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        from lingmo_installer.frontend.kde_components.PartAuto import PartAuto
        from lingmo_installer.frontend.kde_components.PartMan import PartMan
...
class Page(plugin.Plugin):
    def prepare(self):
        ...
        if (self.db.get('lingmo_installer/install_bootloader') == 'true' and
                'LINGMO_INSTALLER_NO_BOOTLOADER' not in os.environ):
        ...
        command = [
            'sh', '-c',
            '/usr/share/lingmo_installer/activate-dmraid && /bin/partman',
        ]
        questions = [
    ...
    def set(self, question, value):
        if question == 'lingmo_installer/partman-rebuild-cache':
        ...
        if 'reuse' in self.extra_options:
            ...
            if len(reuse) == 1:
                ...
                if current_version == new_version and final:
                    ...
                    # are present" case
                    q = 'lingmo_installer/partitioner/lingmo_reinstall'
    ...
    def calculate_autopartitioning_options(self, operating_systems,
        ...
        q = 'lingmo_installer/partitioner/advanced'
        ...
        if os_count == 0:
            ...
            q = 'lingmo_installer/partitioner/multiple_os_format'
            self.db.subst(q, 'DISTRO', release.name)
            title = self.description(q)
            desc = self.extended_description(q)
            ...
            if len(lingmo_systems) == 1:
                ...
                if 'replace' in self.extra_options:
                    q = 'lingmo_installer/partitioner/lingmo_format'
                ...
                if wubi_option:
                    ...
                    q = 'lingmo_installer/partitioner/lingmo_resize'
                    self.db.subst(q, 'DISTRO', release.name)
                ...
                if wubi_option or resize_option or bitlocker:
                    if resize_option or bitlocker:
                        q = 'lingmo_installer/partitioner/single_os_resize'
                    else:
                        q = 'lingmo_installer/partitioner/lingmo_inside'
            ...
            if wubi_option:
                ...
                q = 'lingmo_installer/partitioner/multiple_os_resize'
                self.db.subst(q, 'DISTRO', release.name)
                title = self.description(q)
                desc = self.extended_description(q)
                opt = PartitioningOption(title, desc)
    ...
    def run(self, priority, question):
        ...
        if question.endswith('automatically_partition'):
            ...
            if 'manual' not in self.extra_options:
                ...
                # Here be dragons...
                self.preseed('lingmo_installer/partman-skip-unmount', 'true',
                             seen=False)
            ...
            if self.creating_label:
                response = self.frontend.question_dialog(
                    ...
                    self.extended_description(question),
                    ('lingmo_installer/text/go_back', 'lingmo_installer/text/continue'))
                if response == 'lingmo_installer/text/continue':
            ...
            if do_preseed:
                ...
                self.preseed('lingmo_installer/crypto_key', self.ui.get_crypto_keys())
                self.preseed('lingmo_installer/recovery_key', self.ui.get_recovery_keys())
                return True
            ...
            response = self.frontend.question_dialog(
                ...
                ('lingmo_installer/text/go_back', 'lingmo_installer/text/continue'))
            if response == 'lingmo_installer/text/continue':
                self.db.set('lingmo_installer/partman-confirm', question[8:])
                self.preseed(question, 'true', seen=False)
                self.succeeded = True
                self.done = True
            else:
            ...
            if question == 'partman/unmount_active':
                yes = 'lingmo_installer/imported/yes'
                no = 'lingmo_installer/imported/no'
            elif question == 'partman-efi/non_efi_system':
                yes = 'lingmo_installer/text/in_uefi_mode'
                no = 'lingmo_installer/text/go_back'
            else:
                yes = 'lingmo_installer/text/continue'
                no = 'lingmo_installer/text/go_back'
            response = self.frontend.question_dialog(
                self.description(question),
    ...
    def update_zfs_description(self, description):
        ...
        misc.execute_root('/usr/share/lingmo_installer/zsys-setup', 'layout', device)
        ...
        partlabel = misc.utf8(self.db.metaget('lingmo_installer/text/partman_confirm_zfs', 'extended_description'),
                              errors='replace')
        for line in layout:
    ...
    def ok_handler(self):
        ...
        if self.current_question.endswith('automatically_partition'):
            ...
            self.db.set('lingmo_installer/use_zfs',
            ...
            if keystore_key:
                self.db.set('lingmo_installer/zfs_keystore_key', keystore_key)
