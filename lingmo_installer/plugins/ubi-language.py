...
from lingmo_installer import auto_update, i18n, misc, osextras, plugin
...
try:
    ...
    _ver = '12.04'
_wget_url = 'http://changelogs.lingmo.com/lingmo_installer/%s-update-available' % _ver
...
class PageGtk(PageBase):
    ...
    plugin_title = 'lingmo_installer/text/language_heading_label'
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        builder.add_from_file(os.path.join(
            os.environ['LINGMO_INSTALLER_GLADE'], ui_file))
        ...
        if self.release_notes_label:
            ...
            try:
                ...
                pass
        self.install_lingmo = builder.get_object('install_lingmo')
        self.try_lingmo = builder.get_object('try_lingmo')
        if not self.only:
            if 'LINGMO_INSTALLER_GREETER' not in os.environ:
                choice_section_vbox = builder.get_object('choice_section_vbox')
                choice_section_vbox and choice_section_vbox.hide()
            else:
                def inst(*args):
                    self.try_lingmo.set_sensitive(False)
                    self.controller.go_forward()
                self.install_lingmo.connect('clicked', inst)
                self.try_lingmo.connect('clicked', self.on_try_lingmo_clicked)
    ...
    def on_try_lingmo_clicked(self, *args):
        ...
        # Queue quit.
        self.install_lingmo.set_sensitive(False)
    ...
    def set_language(self, language):
        ...
        if not self.only and 'LINGMO_INSTALLER_GREETER' in os.environ:
            self.try_lingmo.set_sensitive(True)
            self.install_lingmo.set_sensitive(True)
    ...
    def on_language_selection_changed(self, *args, **kwargs):
        ...
        from gi.repository import Gtk
        ltr = i18n.get_string('default-ltr', lang, 'lingmo_installer/imported')
        if ltr == 'default:RTL':
        ...
        # Set the release name (Lingmo 10.04) and medium (USB or CD) where
        # necessary.
        ...
        # Big buttons.
        for w in (self.try_lingmo, self.install_lingmo):
            text = i18n.get_string(Gtk.Buildable.get_name(w), lang)
            text = text.replace('${RELEASE}', release.name)
            text = text.replace('${MEDIUM}', install_medium)
        ...
        self.try_lingmo.set_size_request(-1, -1)
        self.install_lingmo.set_size_request(-1, -1)
        try_w = self.try_lingmo.size_request().width
        install_w = self.install_lingmo.size_request().width
        if try_w > install_w:
            self.try_lingmo.set_size_request(try_w, -1)
            self.install_lingmo.set_size_request(try_w, -1)
        elif install_w > try_w:
            self.try_lingmo.set_size_request(install_w, -1)
            self.install_lingmo.set_size_request(install_w, -1)
...
class PageKde(PageBase):
    plugin_breadcrumb = 'lingmo_installer/text/breadcrumb_language'
    plugin_is_language = True
    ...
    def __init__(self, controller, *args, **kwargs):
        ...
        try:
            ...
            self.page = uic.loadUi('/usr/share/lingmo_installer/qt/stepLanguage.ui')
            ...
            def init_big_button(button, image_name):
                pix = QPixmap('/usr/share/lingmo_installer/qt/images/' + image_name)
                icon = QIcon(pix)
                button.setIcon(icon)
            ...
            def inst(*args):
                self.page.try_lingmo.setEnabled(False)
                self.controller.go_forward()
            self.page.install_lingmo.clicked.connect(inst)
            self.page.try_lingmo.clicked.connect(self.on_try_lingmo_clicked)
            init_big_button(self.page.install_lingmo, 'install.png')
            init_big_button(self.page.try_lingmo, 'try.png')
            ...
            if 'LINGMO_INSTALLER_GREETER' not in os.environ:
                self.page.try_lingmo.hide()
                self.page.try_install_text_label.hide()
                self.page.install_lingmo.hide()
    ...
    def on_try_lingmo_clicked(self, *args):
        ...
        self.controller.allow_change_step(False)
        # Queue quit.
        self.page.install_lingmo.setEnabled(False)
        self.controller._wizard.current_page = None
        self.controller.dbfilter.ok_handler()
    ...
    def openURL(self, url):
        ...
        if os.path.exists("/usr/share/klingmo-default-settings"):
            shutil.copy("/usr/share/klingmo-default-settings/" +
                        "dot-gtkrc-2.0-kde4",
    ...
    def set_language(self, language):
        ...
        if not self.only and 'LINGMO_INSTALLER_GREETER' in os.environ:
            self.page.try_lingmo.setEnabled(True)
            self.page.install_lingmo.setEnabled(True)
    ...
    def on_language_selection_changed(self):
        ...
        if not self.only:
            ...
            for widget in (self.page.try_install_text_label,
                           self.page.try_lingmo,
                           self.page.install_lingmo):
                text = widget.text()
                text = text.replace('${RELEASE}', release.name)
                text = text.replace('${MEDIUM}', install_medium)
...
class Page(plugin.Plugin):
    def prepare(self, unfiltered=False):
        ...
        localechooser_script = '/usr/lib/lingmo_installer/localechooser/localechooser'
        if ('LINGMO_INSTALLER_FRONTEND' in os.environ and
                os.environ['LINGMO_INSTALLER_FRONTEND'] == 'debconf_ui'):
            localechooser_script += '-debconf'

        questions = ['localechooser/languagelist']
        environ = {
            'PATH': '/usr/lib/lingmo_installer/localechooser:' + os.environ['PATH'],
        }
        if ('LINGMO_INSTALLER_FRONTEND' in os.environ and
                os.environ['LINGMO_INSTALLER_FRONTEND'] == "debconf_ui"):
    ...
    def run(self, priority, question):
        if question == 'localechooser/languagelist':
            ...
            only_installable = misc.create_bool(
                self.db.get('lingmo_installer/only-show-installable-languages'))
...
class Install(plugin.InstallPlugin):
    def prepare(self, unfiltered=False):
        if 'LINGMO_INSTALLER_OEM_USER_CONFIG' in os.environ:
            command = ['/usr/share/lingmo_installer/localechooser-apply']
        else:
            command = [
                'sh', '-c',
                '/usr/lib/lingmo_installer/localechooser/post-base-installer ' +
                '&& /usr/lib/lingmo_installer/localechooser/finish-install',
            ]
        return command, []
    ...
    def install(self, target, progress, *args, **kwargs):
        progress.info('lingmo_installer/install/locales')
        rv = plugin.InstallPlugin.install(
            self, target, progress, *args, **kwargs)
        if not rv:
            locale_file = "/etc/default/locale"
            if 'LINGMO_INSTALLER_OEM_USER_CONFIG' not in os.environ:
