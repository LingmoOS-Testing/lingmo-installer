...
def grub_options():
    ...
    from lingmo_installer.parted_server import PartedServer
...
def remove_os_prober_cache():
    osextras.unlink_force('/var/lib/lingmo_installer/os-prober-cache')
    shutil.rmtree('/var/lib/lingmo_installer/linux-boot-prober-cache',
                  ignore_errors=True)
...
def set_indicator_keymaps(lang):
    ...
    from lingmo_installer import gsettings
    ...
    if nodes:
        ...
        # during lingmo_installer-dm, hence if we change that key, we should purge the
        # state that gsd uses, to determine if it should run the
        # conversion. Which are a stamp file, and having the new key set.
        # Ideally, I think lingmo_installer should be more universal and set the new key
...
def is_removable_device(path):
    ...
    # In lingmo_installer environment / is the installation media
    if mp == "/":
        return True
